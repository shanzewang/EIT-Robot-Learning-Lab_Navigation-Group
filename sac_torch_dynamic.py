...
class ReplayBuffer:
    """
    用于SAC算法的简单FIFO经验回放缓冲区，支持行人动态数据
    新增了 time_dynamic_buf 和 time_dynamic2_buf，用于存储当前状态和下一状态的行人数据，形状为 [size, 3, 8, 10]
    store 方法增加了 time_dynamic 和 time_dynamic2 参数
    """
    def __init__(self, obs_dim, act_dim, size, time_steps=3):
        self.obs_buf = np.zeros([size, obs_dim], dtype=np.float32)
        self.obs2_buf = np.zeros([size, obs_dim], dtype=np.float32)
        self.act_buf = np.zeros([size, act_dim], dtype=np.float32)
        self.rew_buf = np.zeros(size, dtype=np.float32)
        self.done_buf = np.zeros(size, dtype=np.float32)
        # 添加行人动态数据的缓冲区，形状为 [size, time_steps, 8, 10]
        self.time_dynamic_buf = np.zeros([size, time_steps, 8, 10], dtype=np.float32)
        self.time_dynamic2_buf = np.zeros([size, time_steps, 8, 10], dtype=np.float32)
        self.ptr, self.size, self.max_size = 0, 0, size

    def store(self, obs, act, rew, next_obs, done, time_dynamic, time_dynamic2):
        self.obs_buf[self.ptr] = obs
        self.obs2_buf[self.ptr] = next_obs
        self.act_buf[self.ptr] = act
        self.rew_buf[self.ptr] = rew
        self.done_buf[self.ptr] = done
        self.time_dynamic_buf[self.ptr] = time_dynamic
        self.time_dynamic2_buf[self.ptr] = time_dynamic2
    ...
    def sample_batch(self, batch_size=128, start=0):
        ...
        batch = dict(
            ...
            time_dynamic=self.time_dynamic_buf[idxs],
            time_dynamic2=self.time_dynamic2_buf[idxs]
        )
...
def sac(
    ...
    def compute_loss_q(data):
        o, a, r, o2, d, time_dynamic, time_dynamic2 = (
            data['obs'], data['act'], data['rew'], data['obs2'], data['done'],
            data['time_dynamic'], data['time_dynamic2']
        )

        q1 = ac.q1(o, a, time_dynamic)
        q2 = ac.q2(o, a, time_dynamic)

        # Bellman备份
        with torch.no_grad():
            a2, logp_a2 = ac.pi(o2, time_dynamic2)
            q1_pi_targ = ac_targ.q1(o2, a2, time_dynamic2)
            q2_pi_targ = ac_targ.q2(o2, a2, time_dynamic2)
    ...
    def compute_loss_pi(data):
        o, time_dynamic = data['obs'], data['time_dynamic']
        pi, logp_pi = ac.pi(o, time_dynamic)
        q1_pi = ac.q1(o, pi, time_dynamic)
        q2_pi = ac.q2(o, pi, time_dynamic)
    ...
    def get_action(o, env=None ,deterministic=False):
        """
        获取动作
        参数：
            o: 原始观测，形状为 [obs_dim]
        ...
        """
        # 获取三帧行人数据，形状为 [3, 8, 10]
        time_dynamic_info = Time_dynamic(env, time_steps=3)

        # 转换为 PyTorch 张量并移到设备
        time_dynamic_tensor = torch.FloatTensor(time_dynamic_info).to(device).unsqueeze(0)
        o_tensor = torch.FloatTensor(o).to(device).unsqueeze(0)

        with torch.no_grad():
            if deterministic:
                pi, _ = ac.pi(o_tensor, time_dynamic_tensor, deterministic=True)
                return pi.squeeze(0).cpu().numpy()
            else:
                a = ac.act(o_tensor, time_dynamic_tensor, deterministic)
                return a.squeeze(0).cpu().numpy()

    def get_dynamic(env):
        """
        从环境中获取动态信息，并处理成一个80维的向量。

        参数:
            env: 环境对象，提供行人信息和机器人尺寸。

        返回:
            dynamic_info: 一个80维的numpy数组，包含所有行人的动态信息（包括机器人的尺寸），
                          如果行人数量少于8个，则用0补全。
        """
        # 获取行人信息，假设返回一个列表，列表中每个元素是字典
        #pedestrians = env.get_pedestrians_info()  # 列表长度 <= 8
        #这里我们测试，硬编码一个行人信息列表
        pedestrians = [
            {'px_i': 0.73, 'py_i': -0.42, 'vx_i': 0.12, 'vy_i': -0.34, 'ri': 0.45, 'rel_dist': 0.84,'rel_angle': -0.52},
            {'px_i': -0.19, 'py_i': 0.87, 'vx_i': -0.23, 'vy_i': 0.41, 'ri': 0.37, 'rel_dist': 0.89, 'rel_angle': 1.79}
        ]


        # 获取机器人尺寸，假设返回 [L1, L2, W]
        #robot_dims = env.get_robot_dimensions()
        robot_dims = [1.0, 2.0, 3.0]   #假设机器人的长宽高 要改
        ...
        for i in range(8):
            if i < len(pedestrians):
                ...
                # 提取每个行人的信息并组合成10维向量
                ped_info = [
                    ped['px_i'], ped['py_i'],  # 行人位置 (2维)
                    ped['vx_i'], ped['vy_i'],  # 行人速度 (2维)
                    ped['ri'],  # 行人半径 (1维)
                    ped['rel_dist'],  # 相对距离 (1维)
                    ped['rel_angle'],  # 相对角度 (1维)
                    robot_dims[0], robot_dims[1], robot_dims[2]  # 机器人长宽高 (3维)
                ]
                # 将该行人的10维信息加入总信息
                dynamic_info.extend(ped_info)
            else:
                # 如果行人不足8个，用0补全10维信息
                dynamic_info.extend([0.0] * 10)

        # 转换为80维numpy数组
        return np.array(dynamic_info)

    def Time_dynamic(env = None, time_steps=3):
        """
        将环境动态信息组织为时间步长的结构

        功能说明：
            从环境中获取动态信息并按时间序列组织，
            当存储的时间步数达到上限时自动移除最旧的帧

        参数说明：
            env: 环境对象，需包含行人信息和机器人尺寸
            time_steps: 时间步长窗口大小，默认为3

        返回值：
            time_dynamic_info: 维度为[time_steps, 8, 10]的numpy数组
                               包含最近time_steps个时间步的动态信息
                               具体结构为[时间步, 行人ID, 行人特征]
        """
        # 初始化固定长度的双端队列存储时间序列
        dynamic_queue = deque(maxlen=time_steps)

        # 从环境获取当前时刻的动态信息(假设为80维向量)
        current_dynamic = get_dynamic(env)

        # 将当前动态信息加入队列尾部
        dynamic_queue.append(current_dynamic)

        # 若队列长度不足time_steps，在队列头部填充零向量
        while len(dynamic_queue) < time_steps:
            dynamic_queue.appendleft(np.zeros_like(current_dynamic))

        # 将队列转换为numpy数组，形状为[time_steps, 80]
        dynamic_array = np.array(dynamic_queue)

        # 将80维特征向量重塑为[8, 10]结构
        # 即8个行人，每人10个特征维度
        dynamic_array = dynamic_array.reshape(time_steps, 8, 10)
    ...
    for hyper_exp in range(1, 5):
        ...
        while test_time < 101:
            ...
            while not reset:
                ...
                try:
                    time_dynamic = Time_dynamic(env, time_steps=3)
                    o2, r, d, goal_reach, r2gd, robot_pose2 = env.step()
                    time_dynamic2 = Time_dynamic(env, time_steps=3)
                    #print(f"训练step成功")
                except Exception as e:
                ...
                #print(f"开始存储到回放缓冲区")
                try:
                    replay_buffer.store(o, a, r, o2, d, time_dynamic, time_dynamic2)
                    #print(f"回放缓冲区存储成功")
                ...
                if episode > start_epoch:
                    if goal_reach == 1 or env.crash_stop or (ep_len >= max_ep_len):
                        ...
                        for j in range(ep_len):  #这里是Q和P网络的更新
                            T += 1
...
if __name__ == '__main__':
    ...
    time.sleep(2)
    print("Roscore 已启动!")
    subprocess.Popen(["rosrun", "stage_ros1", "stageros", "dynamic.world"])
    print("环境已启动!")
    time.sleep(2)
    sac()
