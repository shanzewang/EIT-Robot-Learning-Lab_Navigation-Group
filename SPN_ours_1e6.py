...
def sac(
    ...
    all_trainable_weights_pi = [
        var for var in tf.trainable_variables() if 'main/pi' in var.name
    ]  # 获取可训练变量
    regularization_penalty_pi = tf.add_n(
        [regularizer(var) for var in all_trainable_weights_pi]
    )  # 应用正则化
    ...
    for hyper_exp in range(1, 5):
        ...
        while test_time < 101:
            ...
            while not reset:
                ...
                if d:
                    if episode > start_epoch:
                        suc_record[length_index, int(suc_pointer[length_index])] = (
                            goal_reach
                        )
                        if env.stop_counter < 1.0:
                            suc_record1[
                                length_index, int(suc_pointer[length_index])
                            ] = goal_reach
                            suc_record2[
                                length_index, int(suc_pointer[length_index])
                            ] = 0
                    ...
                    if ep_len >= max_ep_len:
                        ...
                        if episode > start_epoch:
                            ...
                            suc_pointer[length_index] = (
                                suc_pointer[length_index] + 1
                            ) % 50
                            ...
                            if T % 10000 == 0:
                                for shape_no in range(5):
                                    for k in range(50):
                                        ...
                                        ep_len_test = 0
                                        rospy.sleep(2.0)
                                        velcity = env.set_robot_pose_test(
                                            k, int(env_list[shape_no]), shape_no
                                        )
                                        env.GenerateTargetPoint_test(
                                            k, int(env_list[shape_no]), shape_no
                                        )
                                        max_ep_len = int(
                                            40
                                            * 0.8 ** int(env_list[shape_no])
                                            / velcity
                                            * 5
                                        )
                                        o, r, d, goal_reach, r2gd, robot_pose = (
                                            env.step()
                                        )
                                        for i in range(1000):
                                            ...
                                            env.Control(a)
                                            rate.sleep()
                                            o2, r, d, goal_reach, r2gd, robot_pose2 = (
                                                env.step()
                                            )
                                            ...
                                            if d or (ep_len_test >= max_ep_len):
                                                test_result_plot[
                                                    hyper_exp, shape_no, k, test_time, 0
                                                ] = return_epoch_test
                                                test_result_plot[
                                                    hyper_exp, shape_no, k, test_time, 1
                                                ] = goal_reach
                                                test_result_plot[
                                                    hyper_exp, shape_no, k, test_time, 2
                                                ] = (ep_len_test * 1.0 / max_ep_len)
                                                test_result_plot[
                                                    hyper_exp, shape_no, k, test_time, 3
                                                ] = (
                                                    1.0 * goal_reach
                                                    - ep_len_test * 2.0 / max_ep_len
                                                )
                                                test_result_plot[
                                                    hyper_exp, shape_no, k, test_time, 4
                                                ] = env.crash_stop
                                                if k == 49:
                                                    ...
                                                    if shape_no == 4:
                                                        ...
                                                        if (test_time) % 1 == 0:
                                                            trainable_saver.save(
                                                                sess,
                                                                '/home/eit-robot/sz_dclp_ws/src/network_sz/configback540GMMdense'
                                                                + str(sac)
                                                                + 'lambda'
                                                                + str(hyper_exp),
