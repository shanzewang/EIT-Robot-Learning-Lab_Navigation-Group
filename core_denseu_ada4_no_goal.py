...
def mlp_policy(
    x, trainp, hidden_sizes=(32,), activation=tf.nn.leaky_relu, output_activation=None
):
...
def mlp_actor_critic(
    ...
    with tf.variable_scope('pi'):
        ...
        x0 = new_relu(x[:, 0:32], alpha_actv1)
        x0 = tf.concat([x0, x[:, 32:36]], axis=-1)
        mu, pi, logp_pi = policy(
            x0, a, trainp, hidden_sizes, activation, output_activation
        )
    ...
    with tf.variable_scope('q3'):
        ...
        x11 = new_relu(x[:, 0:32], alpha_actv2)
        x111 = tf.concat([x11, x[:, 32:36]], axis=-1)
    vf_mlp = lambda y: tf.squeeze(
        mlp(y, list(hidden_sizes) + [1], activation, None), axis=1
    )
